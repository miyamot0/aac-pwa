...
function populate_software() {
    return licenses.map((relevant_license) => {
        const author =
            relevant_license.author.trim() === 'na'
                ? ''
                : ` ${relevant_license.author}`
        const license = relevant_license?.licenseType ?? 'Error'

        return `${relevant_license.name} (${
            relevant_license.installedVersion
        }). Copyright${author} -- ${license} Licensed: [Repo](${relevant_license.link.replace(
            'git+',
            ''
        )}) `
    })
 ...
 */
function read_md() {
    return fs.readFileSync('./scripts/README.pre.md', 'utf-8')
}
